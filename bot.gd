...
func get_input():
	...
	if keybord_comand.right :
		dir = 1
		$sprite.scale.x = 1
		if velocity.x<300 :
			self.velocity.x += speed*dir
	elif keybord_comand.left :
		dir = -1
		$sprite.scale.x = -1
		if velocity.x>-300 :
			self.velocity.x += speed*dir
...
#		velocity.x += speed*dir
	...
	if keybord_comand.rocket:
		if rockets > 0 : 
			rockets -= 1
			var bomb_instance = bomb.instance()
			var direction = keybord_comand.rocket_direction
...
#		transform.rotated(360)
	if keybord_comand.jump and is_on_floor() :
			is_jumping = true
			$footstep.stop()
